...
def findTargets(contours, frame):
    image = frame.copy()
    ...
    print ("targetCount [" + str(len(targets)) + "]")
    table.getSubTable('Vision').getEntry('targetCount').setValue(len(targets))
    ...
    if (len(targets) > 0):
        ...
        table.getSubTable('Vision').getEntry('angle').setValue(currentAngleError)
